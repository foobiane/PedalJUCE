...
void construct_include_file(char** filenames, int num_files) {
    ...
    if (f) {
        ...
        fprintf(f, "\nconst std::vector<std::string> PEDAL_TYPES = {");
        ...
        for (int i = 0; i < num_files; i++) {
            ...
            fprintf(f, "\"%s\"%s", typename, (i == num_files - 1 ? "" : ","));
            free(typename);
        }

        // Writing a function that allows us to create a pedal dynamically based on a string that 
        // specifies the type
        fprintf(f, "};\n\nstd::unique_ptr<Pedal> getPedalFromName(std::string name, juce::AudioProcessorGraph* g, int editorW, int editorH) {\n");

        for (int i = 0; i < num_files; i++) {
            char* typename = strndup(strrchr(filenames[i], '/') + 1, (int) (strrchr(filenames[i], '.') - strrchr(filenames[i], '/')) - 1);
            fprintf(f, "\tif (name == \"%s\") { return std::make_unique<%s>(g, editorW, editorH); }\n", typename, typename);
            free(typename);
        }
